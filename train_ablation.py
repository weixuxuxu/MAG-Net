...
from models.MVF_ablation import MVF_Net
...
#class_weights = torch.tensor([1.0, 2.0]).to(device)  # 根据类别分布调整权重
#clf_loss_function = nn.CrossEntropyLoss(weight=class_weights)
#clf_loss_function = focal_loss_multi()
...
save_path = '/JingTeam/Shuying/multi_task_XXL/savemodel_ablation/'
...
for epoch in range(max_epochs):
    ...
    # if (epoch + 1) % 50 == 0:
    #     storfile(epoch_loss_values,'/JingTeam/Shuying/multi_task_XXL/savemodel_vd/loss_seg3.csv')
        #storfile(epoch_seg_loss_values,'/JingTeam/Shuying/multi_task_XXL/savemodel/seg_loss.csv')
        #storfile(epoch_clf_loss_values,'/JingTeam/Shuying/multi_task_XXL/savemodel/clf_loss.csv')
        
print(
    f"train completed, best_metric: {best_metric:.4f} best_auc: {best_acc:.4f}")